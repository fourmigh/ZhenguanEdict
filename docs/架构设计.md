...
引擎层定义的通用状态流转（`新建 → 待处理 → 规划中 → 审核中 → 派发中 → 执行中 → 验证中 → 已完成`）是各朝代状态机的基础模板。每个朝代在此基础上简化或扩展特定环节。
...
通用权限矩阵的四个抽象层在各朝代的具体角色对应：

| 抽象层 | 炎黄 | 夏 | 商 | 周 | 秦 | 汉 | 隋唐 | 宋 | 明 | 清 |
|---|---|---|---|---|---|---|---|---|---|---|
| **决策层** | 炎帝/黄帝 | 王 | 王 | 天子 | 皇帝 | 皇帝 | 皇帝/太子 | 皇帝 | 皇帝 | 用户/皇帝 |
| **规划层** | 炎帝 | 王/祭司 | 王/贞人 | 卿士 | 丞相 | 三公（丞相/太尉/御史大夫） | 中书省 | 中书门下/枢密院/三司使 | 内阁 | 军机处/六部 |
