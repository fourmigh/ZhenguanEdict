...
以隋唐三省六部为例，Loop Engineering 可在三个层级引入，层层递进。

### 1. 调度层 Loop（尚书省）

尚书省原本的职责是"一次派发，任务结束"。引入 Loop 后，尚书省转为**迭代调度器**——派发后验证结果，未达标则自动进入下一轮调度：中书省重新规划、门下省重新审核、六部重新执行，直至目标达成或超限。

应用场景：大型跨部门任务（如"完成 TiDB 高可用测试工具"），需要多轮规划-审核-执行才能完成。尚书省充当外循环驱动器，每一轮都基于上一轮的输出继续推进。

### 2. 部门内 Loop（六部）

各部门内部引入自迭代闭环。以兵部（编码型 Agent）为例：
...
这是 Claude Code Hooks 模式的应用——每次文件写入后自动触发验证，把结果喂回 Agent。其他部门同理：工部部署后自动校验，刑部安全扫描后自动修复。

### 3. 任务级 /goal 模式

在任务层面启用**目标驱动循环**，定义明确的验收标准：

```
任务: 完成 TiDB 高可用测试工具
验收标准:
  - 所有 HA 测试场景通过（节点重启/网络延迟/数据损坏）
  - 并发业务模拟成功率 > 99%
  - 输出 Word 测试报告
循环模式: 每轮执行完自动验证，未达标则自动进入下一轮
```

这是 Loop Engineering 中 /goal 闸门机制的体现——它定义了"什么时候停"。系统在每轮结束后自动检查验收标准，达标则归档为已完成，不达标则继续迭代，直到达到最大轮数或时间预算。

### 为什么这很重要

**把"人"移出循环**。传统模式：你写提示词 → AI 回复 → 你看结果 → 你写下一个提示词。你是循环里最慢的那个环节。Loop Engineering 让你设计好系统，Agent 们自己跑，你只需要定义验收标准 + 偶尔检查结果。

**让 Agent 能跑一整夜**。一个能跑 6 小时的 Agent 和一次性的对话式 Agent，是两种完全不同量级的生产力。下班前丢一个复杂任务进去，第二天早上来看结果。

**三省六部走向生产级的最后一块拼图**。三省六部定义了"如何分工、如何制衡"，Loop Engineering 定义了"如何让它自己转起来、什么时候停"。两者结合，就是一套可审计、可制衡、可自主运行的 AI 协作系统。
